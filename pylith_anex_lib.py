        ...
        if cf.has_section(e + '.parameters'):
            for param,val in cf.items(e + '.parameters'):
                ...
                if type(val) is list:
                    ...
                    truemax = max(val[0:2])
                    delta = val[2]
                    configdic[e + '.' +  param] = list(np.arange(truemin,truemax,delta))
                elif type(val) is str:
                    configdic[e + '.' +  param] = [val + '.' + param]
                    #temporary value when the current val have to have another one
                    # change a few lines after
                elif type(val) is float or int :
                    configdic[e + '.' +  param] = [val]              
...
#a , b = configfile2combidic_list(filenameExodus,cfpath)